...
function todoColor(todos, id) {
  let cur = getTodo(todos, id);
  while (cur && cur.parent_id) cur = getTodo(todos, cur.parent_id);
  if (!cur) return TODO_COLORS[0];
  const roots = todos.filter(t => !t.parent_id);
  const idx = roots.findIndex(t => t.id === cur.id);
...
function openModal(title, bodyHtml, onConfirm) {
  ...
  if (!mask) {
    ...
    mask.innerHTML = '<div class="modal"><h3 id="modalTitle"></h3><div id="modalBody"></div><div class="modal-foot" id="modalFoot"></div></div>';
    document.body.appendChild(mask);
    mask.addEventListener('click', e => { if (e.target.id === 'modalMask') closeModal(); });
  }
  document.getElementById('modalTitle').textContent = title;
  document.getElementById('modalBody').innerHTML = bodyHtml;
  document.getElementById('modalFoot').innerHTML = `<button class="btn" id="modalCancel">取消</button><button class="btn primary" id="modalOk">确定</button>`;
...
function renderNav(activeId) {
  const dateStr = (() => { const d = new Date(); return `${d.getMonth()+1}月${d.getDate()}日 周${['日','一','二','三','四','五','六'][d.getDay()]}`; })();
  const sidebarItems = NAV_ITEMS.map(it =>
    `<a class="nav-item ${it.id===activeId?'active':''}" href="${it.href}">${it.icon}${it.label}</a>`
  ).join('');
  const tabItems = NAV_ITEMS.map(it =>
    `<a class="tab-item ${it.id===activeId?'active':''}" href="${it.href}">${it.icon}<span>${it.label}</span></a>`
  ).join('');
  return `
    <aside class="sidebar">
      <div class="brand"><h1>我的工作台</h1><p>${dateStr}</p></div>
      <nav class="nav">${sidebarItems}</nav>
    </aside>
    <div style="flex:1;min-width:0;">
      <div class="topbar"><h1>我的工作台</h1><span class="date">${dateStr}</span></div>
  `;
}
function renderTabbar(activeId) {
  ...
  return `<nav class="tabbar">${tabItems}</nav>`;
}
